...
/**
 * 帶 429 重試的 Gemini API 呼叫
 * @param {Function} fn - 要執行的 async 函式
 * @param {number} maxRetries - 最大重試次數（預設 3）
 * @returns {Promise}
 */
async function retryOnQuotaExceeded(fn, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // 嘗試從錯誤訊息解析建議等待秒數，例如 "retry in 59.888119352s"
      const retryMatch = msg.match(/retry\s+in\s+([\d.]+)s/i);
      const waitSec = retryMatch ? Math.ceil(parseFloat(retryMatch[1])) + 2 : 30 * attempt;
      const waitMs = Math.min(waitSec * 1000, 120_000); // 最多等 120 秒

      console.log(`[retry] Gemini 429 配額限制，第 ${attempt}/${maxRetries} 次重試，等待 ${waitSec} 秒...`);
...
async function parseBusinessCardsBatch(pdfBuffer) {
  ...
  try {
    // 上傳也可能被限流，加入重試
    const uploadResult = await retryOnQuotaExceeded(() =>
      fm.uploadFile(tmpPath, {
    ...
    // 3. 傳給 Gemini 分析（帶 429 重試）
    console.log('[scan-batch] 送出 AI 辨識請求...');
    const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
    const result = await retryOnQuotaExceeded(() =>
