...
router.post('/inspect-selector', requireRole('super_admin', 'dept_admin'), async (req, res) => {
...
router.post('/', requireRole('super_admin', 'dept_admin'), async (req, res) => {
...
router.post('/:id/test', requireRole('super_admin', 'dept_admin'), async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM crm_connections WHERE id=$1', [req.params.id]);
    if (!result.rows[0]) return res.status(404).json({ error: '連線不存在' });
